...
public class ComputeAndPersistDaysSinceLastCommitStep implements ComputationStep {
  ...
  public ComputeAndPersistDaysSinceLastCommitStep(DbClient dbClient, SourceLineIndex sourceLineIndex, MetricCache metricCache) {
  ...
  private void persistNumberOfDaysSinceLastCommit(ComputationContext context) {
    ...
    long numberOfDaysSinceLastCommit = (System.currentTimeMillis() - lastCommitTimestamp) / MILLISECONDS_PER_DAY;
    ...
    try {
      dbClient.measureDao().insert(dbSession, new MeasureDto()
        ...
        .setComponentId(context.getProject().getId()));
      dbSession.commit();
    } finally {
